...
export default function Games() {
  ...
  const loadArticles = useCallback(async () => {
    setIsLoading(true);
    const data = await Article.filter({ type: "game" }, "-created_date");
    setArticles(data);
    setIsLoading(false);
  }, []);
  ...
  const filterArticles = useCallback(() => {
    ...
    if (selectedPlatform !== "all") {
      filtered = filtered.filter(article =>
        article.platform && article.platform.includes(selectedPlatform)
      );
    }
  ...
  }, [articles, searchTerm, selectedGenre, selectedPlatform, selectedStatus, sortBy]);
  ...
  const allPlatforms = [...new Set(articles.flatMap(article => article.platform || []))];
